...
export function SiriWave({
  ...
  React.useEffect(() => {
    ...
    // A reader who asked for no motion gets a single settled frame instead of
    // an animation loop -- the shader still draws, it just stops advancing.
    const reduced =
    ...
    // Off-screen frames are wasted GPU time on a page with three live shaders,
    // so the loop parks itself until the canvas is actually in view.
    let visible = true
    ...
    const frame = () => {
      const now =
        typeof performance !== "undefined" ? performance.now() : Date.now()
      if (visible) drawAt((now - start) / 1000)
